...
class Philosopher(threading.Thread):
    ...
    def run(self):
        while (self.running):
            ...
            time.sleep(30)
            print('Philosopher %s is hungry.' % self.index, flush=True)
            self.dine()
    ...
    def dine(self):
        ...
        while self.running:
            ...
            fork1.release()
            print('Philosopher %s swaps forks.' % self.index, flush=True)
    ...
    def dining(self):
        print('Philosopher %s starts eating. ' % self.index, flush=True)
        time.sleep(30)
        print('Philosopher %s finishes eating and leaves to think.' % self.index, flush=True)
